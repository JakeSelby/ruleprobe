...
no `--plugins`, because `ruleprobe corpus` loads none to score the negative with. A Claude
Code subagent's own transcript is its own session, keyed by its agent id, and a transcript
that yields no event is no session at all. When two transcripts still share a session id, the
session with the hit at the key is the one taken. It prints the `ruleprobe corpus` command that scores the new negative, with the
