...
- **Re-run after #87** (2026-09-24, main at `5464b32`, which adds the copied-session pre-scan): the
  same command on the same volume, three runs with no bytecode cache (`python3 -B`) and three with
  `--json`, on the same machine. Worst 1.27 s without `--json` (the first run; the others 0.90 s
  and 0.93 s) and 0.96 s with it. Still within 60 s; the pre-scan leaves the verdict unchanged.
